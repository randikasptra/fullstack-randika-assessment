                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                            Keranjang Belanja ({loading ? 0 : cartItems.length} item)
                        </h1>
                    </div>
                    ...
                    <>
                            ...
                            {displayItems.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="flex-shrink-0 mt-1">
                                        <input
                                            type="checkbox"
                                            checked={loading ? false : selectedItems.has(item.id)}
                                            onChange={() => !loading && toggleItemSelection(item.id)}
                                            disabled={loading}
                                    ...
                                    <img
                                        src={
                                            loading
                                                ...
                                                : (item.book.image_url || "https://images.unsplash.com/photo-1524995997946-a1c2e315a42f")
                                        }
                                        alt={item.book.title}
                                        className={`w-20 h-28 object-cover rounded-lg flex-shrink-0 ${loading ? 'animate-pulse bg-gray-200' : ''}`}
                                    />
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                                            {loading ? "" : (item.book.author || "Tidak diketahui")}
                                        </p>
                                        <p className={`text-lg font-bold ${loading ? 'text-gray-400' : 'text-green-700'} mb-3`}>
                                            {loading ? "Rp 0" : `Rp ${item.book.price.toLocaleString("id-ID")}`}
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4 flex-shrink-0">
                                        <div className="flex items-center gap-2">
                                            <button
                                                onClick={() => !loading && handleUpdateQuantity(item.id, item.quantity - 1)}
                                                disabled={loading}
                                                className={`p-1 ${loading ? 'text-gray-300 cursor-not-allowed' : 'text-gray-500 hover:text-gray-700'}`}
                                            >
                                                <Minus className="w-4 h-4" />
                                            </button>
                                            <span className={`w-8 text-center font-medium ${loading ? 'text-gray-400' : ''}`}>
                                        ...
                                        <button
                                            onClick={() => !loading && handleRemoveItem(item.id)}
                                            disabled={loading}
                                            className={`p-2 ${loading ? 'text-gray-300 cursor-not-allowed' : 'text-red-500 hover:text-red-700'}`}
                                        >
                                            <Trash2 className="w-5 h-5" />
                                        </button>
                                    </div>
                                    <div className="text-right flex-shrink-0 ml-auto">
                                        <p className={`font-bold ${loading ? 'text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                                            {loading ? "Rp 0" : `Rp ${(item.book.price * item.quantity).toLocaleString("id-ID")}`}
                                        </p>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={() => !loading && toggleSelectAll()}
                                        disabled={loading}
                                        className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 disabled:opacity-50"
                                    />
                                    <label className={`text-sm font-medium ${loading ? 'text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                        {loading ? "Memuat..." : `Pilih Semua (${selectedItems.size}/${cartItems.length})`}
                                    </label>
                                </div>
                                <span className={`text-xl font-bold ${loading ? 'text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                                    Total: {loading ? "Rp 0" : `Rp ${total.toLocaleString("id-ID")}`}
                                </span>
                            </div>
                            <button
                            ...
                            >
                                ...
                                {loading ? "Memuat..." : `Lanjut Checkout (${selectedItems.size} item)`}
                            </button>
                        </div>
                    </>
                )}
