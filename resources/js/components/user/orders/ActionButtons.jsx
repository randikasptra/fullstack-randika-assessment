...
const ActionButtons = ({ order, onCancel, onConfirm, onDelete, actionLoading }) => {
    const navigate = (path) => window.location.href = path; // Simple, or use useNavigate if parent
    ...
    const getActions = () => {
        switch (order.status) {
            ...
            case "paid":
                return <button className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-xl font-medium transition-all">Pesanan Diproses</button>;
            case "shipped":
                return (
                    <button
